...
def init_client(credentials_dict: Dict) -> bigquery.Client:
    ...
    return bigquery.Client(credentials=credentials, project=credentials_dict['project_id'])


def load_items_to_embed(client: bigquery.Client, dataset_id: str = DATASET_ID, shuffle: bool = False) -> bigquery.table.RowIterator: 
    query = f"""
    SELECT 
    item.*, 
